...
- **対象**: 新規`packages/backend/emit/src/disasm/x86-64-disasm.lisp`, `packages/backend/emit/src/disasm/aarch64-disasm.lisp`
- **現状**: バイトコードISA逆アセンブラ（`packages/backend/bytecode/src/decode.lisp:202`、`disassemble-instruction`）は存在。x86-64/ARM64のネイティブ命令デコーダなし
...
- **対象**: `packages/engine/compile/src/codegen.lisp`, `packages/frontend/parse/src/diagnostics.lisp`
...
- **対象**: `packages/engine/vm/src/vm-run.lisp`, 新規`src/jit/baseline.lisp`
- **現状**: VMはフラットベクタのインタープリタ（`run-vm`、`vm-run.lisp:265`）。ネイティブコード生成は`packages/backend/emit/src/x86-64-codegen.lisp`に存在するがVM実行とは分離されている
...
- **対象**: `packages/engine/vm/src/vm.lisp`, `packages/engine/vm/src/vm-run.lisp`, `src/jit/baseline.lisp`
...
- **対象**: `src/jit/`, `packages/engine/vm/src/vm.lisp`
...
- **対象**: `packages/engine/vm/src/vm-execute.lisp`, `src/jit/baseline.lisp`
...
- **対象**: `packages/engine/optimize/src/optimizer.lisp`, 新規`packages/engine/optimize/src/ipa.lisp`
...
- **対象**: `packages/engine/optimize/src/optimizer.lisp`, `packages/backend/binary/src/macho.lisp`
...
- **対象**: 新規`packages/engine/optimize/src/alias.lisp`, `packages/engine/optimize/src/optimizer.lisp`
...
- **対象**: 新規`packages/engine/optimize/src/points-to.lisp`
...
- **対象**: 新規`packages/engine/compile/src/simd-builtins.lisp`, `packages/engine/compile/src/builtin-registry.lisp`
- **現状**: `builtin-registry.lisp`の~210エントリにSIMD演算なし
- **内容**: `(cl-cc:simd:f32x4-add a b)` 形式の明示的SIMDイントリンシック。`f32x4`, `f64x2`, `i32x8`等のSIMDベクタ型。`packages/engine/compile/src/simd-builtins.lisp`でbuiltinエントリとして登録。CL配列↔SIMDベクタ変換ヘルパ
...
- **対象**: `packages/engine/compile/src/codegen.lisp`, テストフレームワーク
