...
> 注: `新ファイル` とある項目は、現時点では実装予定を示す。実在するモジュール名は `packages/type/type/src/` 配下の現行ファイルに合わせて読むこと。
...
- **対象**: `packages/type/type/src/bidirectional.lisp` / `packages/type/type/src/checker.lisp`
...
- **活用**: `packages/type/type/src/inference.lisp` の `extract-type-guard` が既に型絞り込みを実装済み → codegen 側で活用
...
- **対象**: `packages/engine/vm/src/primitives.lisp`, `packages/type/type/src/subtyping.lisp`
...
- **対象**: `packages/engine/vm/src/primitives.lisp`, `packages/type/type/src/inference.lisp`
- **内容**:
