...
| `the` | ✅ | |
| `declare` | ✅ | 全宣言形式を受理 (`type`, `optimize`, `ftype`, `ignore`, `ignorable`, `special` 等); 効果は `ignore`/`type` のみ — `optimize`/`ftype` は受理するが伝播なし (ANSI 準拠: 実装は宣言を無視してよい) |
...
| `make-load-form` / `make-load-form-saving-slots` | ✅ | FR-550: GF 定義; 数値・文字・シンボル・文字列は自己表現返却; CLOS は `make-instance` 形式返却 |
...
| `define-modify-macro` | ✅ | |
| `defsetf` (短/長形式) | ✅ | FR-355: 短形式 `(defsetf acc upd)` + 長形式 `(defsetf acc (args) (store) body)` 対応; `*setf-compound-place-handlers*` に登録 |
...
- **対象**: `src/vm/primitives.lisp`, `src/compile/builtin-registry.lisp`
- **現状**: `stdlib-source.lisp` で実装済み (数値・文字・文字列・cons・vector・hash-table)
...
| `defgeneric` (基本) | ✅ | |
| `defgeneric` オプション | ✅ | FR-382: `(:method ...)` 展開・`:documentation`/`:argument-precedence-order`/`:generic-function-class` 受理; `:method-combination` 対応 (短形式カスタム組合せ実装済み) |
| `defmethod` (primary) | ✅ | |
...
| `shared-initialize` GF | ✅ | FR-1005: `:__initargs__` マップを走査して slot-names でフィルタ; 機能的に ANSI 準拠 (MOP 内部は簡略) |
| `reinitialize-instance` GF | ✅ | FR-1005: `:__initargs__` マップを走査して plist で一致するスロットを更新; 機能的に ANSI 準拠 (MOP 内部は簡略) |
...
| `define-method-combination` | ✅ | FR-376: 短形式実装 — `define-method-combination` マクロ + `defgeneric :method-combination` パーサー対応; VM ディスパッチが CPL 全走査で適用メソッド収集 → オペレータ fold; `+`, `*`, `list`, `append`, `nconc`, `and`, `or`, `progn`, `max`, `min` 対応; 長形式は未対応 |
...
| `#S(...)` 読み取り構文 | ✅ | FR-556: `lex-read-hash-dispatch` で `#S` → ホスト CL `read` に委譲して構造体リテラルを読み取り |
...
| `:documentation` | ✅ | パーサーで受理; 保存・参照なし (ANSI では保存は実装定義) |
...
| `restart-bind` | ✅ | FR-692: `*%active-restarts*` にリスタートを動的束縛 |
...
| `with-simple-restart` | ✅ | FR-422: `macros-stdlib.lisp` でスタブ実装 |
| `with-condition-restarts` | ✅ | FR-423: `macros-stdlib.lisp` でスタブ実装 |
| `restart-case :interactive` / `:report` / `:test` オプション | ✅ | FR-420: オプション解析済み (body から分離); `:report`/`:interactive`/`:test` は構文受理するが実行時未接続 |
...
| `assert` | ✅ | FR-606: `cerror` による continuable error; `:place` リストは非対応だが ANSI 基本形は完備 |
...
| `float` (1引数: 変換) | ✅ | |
| `float` (2引数: `(float x prototype)`) | ✅ | FR-604: expander でプロトタイプ引数を無視し `(float x)` に展開 (NaN-boxing double統一) |
