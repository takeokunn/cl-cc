...
| `compile-file` | ✅ | `packages/frontend/expand/src/macros-stdlib.lisp: compile-file` shim; `our-load` を呼び、3値を返す |
...
- **対象**: `packages/engine/vm/src/vm.lisp`, `packages/engine/compile/src/builtin-registry.lisp`
...
- **対象**: `packages/engine/vm/src/primitives.lisp`, `packages/engine/compile/src/builtin-registry.lisp`
...
- **対象**: `packages/engine/vm/src/primitives.lisp` の `vm-typep-check`, `packages/engine/compile/src/codegen.lisp`
...
- **対象**: `packages/engine/compile/src/codegen.lisp`, `packages/foundation/ast/src/ast.lisp`
...
- **対象**: `packages/frontend/expand/src/loop.lisp`
...
| 要素 | cl-cc | SBCL | 備考 |
|------|-------|------|------|
| method group specification | ✅ FR-376 | ✅ | `((primary ()) (around (:around)))` のような節定義 |
| `call-method` | ✅ FR-376 | ✅ | 展開結果の中で個別メソッドを呼ぶ |
| `make-method` | ✅ FR-376 | ✅ | 長形式でネストしたメソッドフォームを組み立てる |
...
| `make-instances-obsolete` | — | ✅ | cl-cc で実装済みと断定できる repo 根拠はこの版では採用していない |
| `update-instance-for-redefined-class` GF | — | ✅ | cl-cc で実装証拠を確認できなかった |
...
| `:metaclass` を実働させる公開プロトコル | — | ✅ | cl-cc では `ensure-class` が主に `defclass` へ委譲し、専用メタクラス API の根拠は未確認 |
| `funcallable-standard-class` | — | ✅ | cl-cc 内で同等の公開メタクラス API の根拠は未確認 |
...
cl-cc は `defstruct` を `defclass` への変換として実装 (`packages/frontend/expand/src/expander-defstruct.lisp`)。
...
| `make-instances-obsolete` | — | ✅ | cl-cc で実装済みと断定できる根拠はこのファイルから外した |
| `update-instance-for-redefined-class` | — | ✅ | cl-cc で実装証拠を確認できなかった |
| クラス再定義時の遅延マイグレーション | — | ✅ | SBCL の代表的挙動 |
...
| `method-qualifiers` 等の公開 MOP accessor | — | ✅ | cl-cc で同等 accessor の根拠は未確認 |
...
- **cl-cc 実装**: `packages/engine/vm/src/vm-clos.lisp`, `packages/engine/compile/src/codegen-clos.lisp`, `packages/engine/vm/src/vm-execute.lisp:383-457`
