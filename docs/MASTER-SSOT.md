...
> **コンパイラコレクション** - Pure Common Lispによる高いコンパイラ技術の実現

CL-CCは、コンパイラ技術の頂点を極める野心的プロジェクトです。外部依存ゼロ、CLOS駆動設計、マクロメタプログラミング、S式Prolog統合、Property-Based Testingという5つの支柱により、従来のコンパイラの限界を超越します。
