...
コンパイラは、プログラミング言語の実装における最も複雑で魅力的なソフトウェアの一つです。本章では、CL-CCが目指す「コンパイラコレクション」を実現するための理論的基盤を解説します。
