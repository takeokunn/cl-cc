...
- **対象**: `src/emit/wasm-types.lisp`, `src/vm/vm-clos.lisp`
...
- **対象**: `src/emit/wasm-trampoline.lisp`, `src/vm/list.lisp`
...
- **対象**: `src/emit/wasm.lisp`, `src/runtime/heap.lisp`
...
- **対象**: `src/emit/wasm.lisp`, 数値演算エミッタ
...
- **対象**: `src/emit/wasm.lisp`, `src/compile/codegen.lisp`
...
- **対象**: `src/emit/wasm-trampoline.lisp`, 型述語エミッタ
...
- **対象**: `src/emit/wasm-trampoline.lisp`, `src/expand/macros-stdlib.lisp`
...
- **対象**: `src/vm/strings.lisp`, `src/emit/wasm-trampoline.lisp`
...
- **対象**: `src/cli/main.lisp`, `src/emit/wasm.lisp`
...
- **対象**: `src/optimize/optimizer.lisp`, `src/emit/wasm-trampoline.lisp`
...
- **対象**: `src/emit/wasm.lisp`, `src/vm/vm.lisp`
...
- **対象**: `src/emit/wasm-trampoline.lisp`, `src/vm/vm.lisp`
...
- **対象**: `src/cli/main.lisp`, `src/emit/wasm.lisp`, `src/vm/vm.lisp`
...
- **対象**: `src/emit/wasm.lisp`, `src/vm/list.lisp`, `src/vm/strings.lisp`
...
- **対象**: `src/vm/primitives.lisp`, `src/emit/wasm-trampoline.lisp`
...
- **対象**: `src/emit/wasm-trampoline.lisp`, `src/vm/primitives.lisp`
...
- **対象**: `src/compile/codegen.lisp`, `src/emit/wasm-trampoline.lisp`
...
- **対象**: `src/runtime/heap.lisp`, `src/emit/wasm.lisp`
- **現状**: Wasm のメモリページサイズは固定 64KB。CL の小オブジェクト主体のヒープには粒度が粗すぎる
- **内容**: Wasm Custom Page Sizes Proposal（Phase 3、2024年）でページサイズを `4096`（4KB）または `1`（1B）に設定可能。GC ヒープを 4KB ページで管理し、`memory.grow` の最小単位を OS の mmap と揃える。世代別GC（`src/runtime/gc.lisp`）のページビットマップが 16倍細粒度化
...
- **対象**: `src/runtime/gc.lisp`, `src/emit/wasm.lisp`
...
- **対象**: `src/emit/wasm.lisp`, FR-227 の動的リンク基盤
...
- **対象**: `src/emit/wasm.lisp`, SIMD エミッタ
...
- **対象**: `src/emit/wasm.lisp`, 浮動小数点エミッタ
...
- **対象**: `src/emit/wasm-trampoline.lisp`, `src/vm/array.lisp`
...
- **対象**: `src/vm/strings.lisp`, `src/vm/array.lisp`
- **現状**: Wasm GC 配列の要素アクセスは `array.get` で1要素ずつ。文字列スキャン・シリアライザは ループで複数要素を個別取得
- **内容**: Wasm Multibyte Array Access Proposal（Phase 1）の `array.load2_u`（2要素を i32 にパック）/ `array.load4_u`（4要素を i32 に）/ `array.store2`/`array.store4` を使用。UTF-16 文字列操作・バイナリシリアライゼーション（Mach-O builder `src/binary/macho.lisp` 等）でアクセス命令数を 1/2〜1/4 に削減
...
- **対象**: `src/emit/wasm-trampoline.lisp`, `src/vm/conditions.lisp`
...
- **対象**: `src/emit/wasm-trampoline.lisp`, `src/compile/cps.lisp`
...
- **根拠**: CPS 変換（`src/compile/cps.lisp`）後のコードは継続への末尾呼び出しが支配的。`return_call_ref` なしでは深い再帰がスタック枯渇する
- **難易度**: Medium
...
- **対象**: `src/vm/conditions.lisp`, `src/runtime/gc.lisp`, `src/emit/wasm.lisp`
...
- **対象**: `src/runtime/gc.lisp`, `src/emit/wasm-trampoline.lisp`
...
- **対象**: `src/emit/wasm.lisp`, `src/vm/io.lisp`
...
- **対象**: `src/emit/wasm.lisp`, `src/optimize/optimizer.lisp`
...
- **対象**: `src/optimize/optimizer.lisp`, `src/emit/wasm.lisp`
...
- **対象**: `src/vm/conditions.lisp`, `src/emit/wasm-trampoline.lisp`
...
- **対象**: `src/emit/wasm-trampoline.lisp`, `src/vm/vm-clos.lisp`
...
- **対象**: `src/emit/wasm.lisp`, `src/vm/io.lisp`, `src/cli/main.lisp`
...
- **対象**: `src/emit/wasm-types.lisp`, `src/vm/vm.lisp`
...
- **対象**: `src/emit/wasm-types.lisp`, `src/vm/strings.lisp`
...
- **対象**: `src/emit/wasm-trampoline.lisp`, `src/vm/vm-execute.lisp`
...
- **対象**: `src/cli/main.lisp`, `src/emit/wasm.lisp`, `src/compile/codegen.lisp`
...
- **対象**: `src/vm/primitives.lisp`, `src/emit/wasm.lisp`
...
- **対象**: `src/emit/wasm.lisp`, `src/vm/conditions.lisp`
...
- **対象**: `src/emit/wasm.lisp`, SIMD エミッタ, `tests/unit/vm/primitives-tests.lisp`
