...
**実装状況**: 175 FR を追跡中。各FR見出しの `✅` は実装済み証跡が揃った項目、`🔶` は helper・部分統合・設計証跡までの項目を示す。`packages/optimize/src/optimizer-pipeline.lisp` の roadmap evidence registry は、この状態区分と公開API・テストアンカーの対応を検証する。
