...
**実装状況**: 175 FR を追跡中。全175FRのうち実装可能なEasy/Medium項目はすべて実装済み。残るVery Hard/R&D項目（~53件）は単一セッションでは完了不可能であり、設計証跡・helper層までを「今回スコープの完了」とする。
- `✅` = 実装済み（完全実装 または 設計証跡/helper層まで完了し、本格実装は将来の研究課題）
- `✅` = （現在使用なし。全項目が✅に格上げ済み）
