...
(define-vm-instruction vm-list (vm-instruction)
  "Create a list from COUNT values in SRC-REGS, store in DST.
The first element in SRC-REGS becomes the first element of the list."
  (dst nil :reader vm-dst)
  (count nil :reader vm-list-count)
  (src-regs nil :reader vm-src-regs)
  (:conc-name vm-list-))

;; vm-list uses list* in its sexp (splicing src-regs), so keep manual method
