...
(eval-when (:compile-toplevel :load-toplevel :execute)
  ...
  ;; Re-export parse symbols from :cl-cc so cl-cc:SYMBOL syntax works
  ;; in downstream packages (e.g. :cl-cc/cli, tests).
  (do-external-symbols (s :cl-cc/parse)
    (export s :cl-cc))
  ;; Re-export expand symbols from :cl-cc so cl-cc:SYMBOL syntax works
  ;; in downstream packages (e.g. :cl-cc/cli).
  (do-external-symbols (s :cl-cc/expand)
    (export s :cl-cc))
  ;; Re-export compile symbols from :cl-cc so cl-cc:SYMBOL syntax works
  ;; in downstream packages (e.g. :cl-cc/cli, tests).
  (do-external-symbols (s :cl-cc/compile)
    (export s :cl-cc))
  ;; Wire VM source files into the umbrella package
  (use-package :cl-cc :cl-cc/vm)
  ;; Re-export VM symbols from :cl-cc so cl-cc:SYMBOL syntax works
  ;; in downstream packages (e.g. :cl-cc/cli, tests).
