...
(defun vm-typep-check (value type-sym)
  ...
  (cond
    ...
    ((eq type-sym 'real) (realp value))
    ((eq type-sym 'rational) (rationalp value))
    ((eq type-sym 'complex) (complexp value))
    ((eq type-sym 'string) (stringp value))
    ((eq type-sym 'symbol) (symbolp value))
    ((eq type-sym 'keyword) (keywordp value))
    ((eq type-sym 'cons) (consp value))
    ((eq type-sym 'null) (null value))
    ((eq type-sym 'list) (listp value))
    ((eq type-sym 'number) (numberp value))
    ((member type-sym '(character base-char standard-char)) (characterp value))
    ((eq type-sym 'bit) (or (eql value 0) (eql value 1)))
    ((eq type-sym 'bit-vector) (typep value 'bit-vector))
    ((eq type-sym 'simple-vector) (typep value 'simple-vector))
    ((eq type-sym 'atom) (atom value))
    ((eq type-sym 'function) (functionp value))
    ((eq type-sym 'vector) (vectorp value))
    ((eq type-sym 'simple-array) (typep value 'simple-array))
    ((eq type-sym 'array) (arrayp value))
    ((eq type-sym 'package) (packagep value))
    ;; FR-598: stream type specifiers
    ((eq type-sym 'stream) (streamp value))
    ((eq type-sym 'input-stream) (input-stream-p value))
    ((eq type-sym 'output-stream) (output-stream-p value))
    ((eq type-sym 'file-stream) (typep value 'file-stream))
    ((eq type-sym 'string-stream) (typep value 'string-stream))
    ((eq type-sym 'broadcast-stream) (typep value 'broadcast-stream))
    ((eq type-sym 'two-way-stream) (typep value 'two-way-stream))
    ((eq type-sym 'echo-stream) (typep value 'echo-stream))
    ((eq type-sym 'concatenated-stream) (typep value 'concatenated-stream))
    ((eq type-sym 'synonym-stream) (typep value 'synonym-stream))
    ;; VM hash tables are wrapped in vm-hash-table-object
    ((eq type-sym 'hash-table) (typep value 'vm-hash-table-object))
    ((and (consp type-sym) (string-equal (symbol-name (car type-sym)) "REFINE"))
     (let ((base (second type-sym))
           (predicate (third type-sym)))
       (and (vm-typep-check value base)
            (cond
              ((functionp predicate) (ignore-errors (funcall predicate value)))
    ...
    ((type-refinement-p type-sym)
     (and (vm-typep-check value (type-refinement-base type-sym))
          (let ((predicate (type-refinement-predicate type-sym)))
            (cond
              ((functionp predicate) (ignore-errors (funcall predicate value)))
              ((and (symbolp predicate) (fboundp predicate))
               (ignore-errors (funcall (symbol-function predicate) value)))
...
(defmethod execute-instruction ((inst vm-type-of) state pc labels)
  ...
  (let* ((value (vm-reg-get state (vm-src inst)))
           (result (cond
                      ...
                      ((typep value 'bignum) 'bignum)
                      ((integerp value) 'integer)
                      ((typep value 'ratio) 'ratio)
                      ((typep value 'double-float) 'double-float)
                      ((typep value 'long-float) 'long-float)
                      ((or (typep value 'single-float)
                           (typep value 'short-float)
                           (floatp value)) 'single-float)
                     ((typep value 'bit-vector) 'bit-vector)
                     ((stringp value) 'string)
                     ((typep value 'simple-vector) 'simple-vector)
                     ((typep value 'simple-array) 'simple-array)
                     ((characterp value) 'character)
                    ((symbolp value) 'symbol)
                     ((pathnamep value) 'pathname)
                     ((typep value 'random-state) 'random-state)
                     ((readtablep value) 'readtable)
