...
(defun %binding-mentioned-in-node-p (node binding-name)
  (typecase node
    (ast-var (eq (ast-var-name node) binding-name))
    (t (some (lambda (child)
               (%binding-mentioned-in-node-p child binding-name))
             (ast-children node)))))

(defun %binding-mentioned-in-body-p (body-forms binding-name)
  (and (listp body-forms)
       (some (lambda (form) (%binding-mentioned-in-node-p form binding-name))
             body-forms)))
...
(defun %ast-let-sink-array-if-candidate (node)
  (let* ((bindings (ast-let-bindings node))
         (body (ast-let-body node)))
    (when (and (= (length bindings) 1)
               (= (length body) 1)
      ...
      (let* ((binding (first bindings))
        ...
        (when (and (%ast-make-array-call-p expr)
                   (not (%ast-make-array-int-call-p expr))
                   (not (and then-uses else-uses)))
          (cond
            ((and then-uses (not else-uses)
                  (not (binding-escapes-in-body-p then-forms name)))
             (make-ast-if :cond (ast-if-cond if-node)
...
(defmethod compile-ast ((node ast-let) ctx)
  (let ((sunk (%ast-let-sink-array-if-candidate node)))
    (when sunk
