...
(deftest cps-transform-int
  (let ((fn (cps-transform-eval '42)))
    (assert-equal 42 (funcall fn #'identity))))

(deftest cps-transform-if
  (let ((fn (cps-transform-eval '(if 1 10 20))))
    (assert-equal 10 (funcall fn #'identity))))

(deftest cps-transform-if-false
  (let ((fn (cps-transform-eval '(if 0 10 20))))
    (assert-equal 20 (funcall fn #'identity))))

(deftest cps-transform-progn
  (let ((fn (cps-transform-eval '(progn 1 2 3))))
    (assert-equal 3 (funcall fn #'identity))))

(deftest cps-transform-let
  (let ((fn (cps-transform-eval '(let ((x 1) (y 2)) (+ x y)))))
    (assert-equal 3 (funcall fn #'identity))))

(deftest cps-transform-nested-let
  (let ((fn (cps-transform-eval '(let ((x 1) (y (+ x 1))) (let ((z 3)) (+ y z))))))
    (assert-equal 5 (funcall fn #'identity))))
...
(deftest cps-transform-ast-progn
...
(deftest cps-transform-ast-print
...
(deftest cps-transform-ast-lambda
...
(deftest cps-transform-ast-block
  (let* ((body (list (cl-cc:make-ast-int :value 42)))
...
(deftest cps-transform-ast-return-from
...
(deftest cps-transform-ast-tagbody
...
(deftest cps-transform-ast-go
  (let* ((ast (cl-cc:make-ast-go :tag 'start))
         (result (cps-transform-ast* ast)))
    (assert-true (listp result))
    (assert-eq 'lambda (car result))))

(deftest cps-transform-ast-catch
...
(deftest cps-transform-ast-throw
  (let* ((tag (cl-cc:make-ast-var :name 'my-tag))
...
(deftest cps-transform-ast-unwind-protect
...
(deftest cps-transform-ast-flet
...
(deftest cps-transform-ast-labels
