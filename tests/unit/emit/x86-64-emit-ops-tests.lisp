...
(deftest x86-emit-const-integer-emits-bytes
  ...
  (let* ((inst (make-vm-const :dst :r0 :value 42))
         (bytes (%collect-emit-ops-bytes #'cl-cc::emit-vm-const inst)))
...
(deftest x86-emit-move-between-gp-regs
  ...
  (let* ((inst (make-vm-move :dst :r0 :src :r1))
         (bytes (%collect-emit-ops-bytes #'cl-cc::emit-vm-move inst)))
...
(deftest-each x86-emit-binary-alu-emits-bytes
  ...
  :cases (("add" #'cl-cc::emit-vm-add (make-vm-add :dst :r0 :lhs :r1 :rhs :r2))
          ("sub" #'cl-cc::emit-vm-sub (make-vm-sub :dst :r0 :lhs :r1 :rhs :r2))
          ("mul" #'cl-cc::emit-vm-mul (make-vm-mul :dst :r0 :lhs :r1 :rhs :r2)))
  (emit-fn inst)
...
(deftest x86-emit-div-emits-34-bytes
  ...
  (let* ((inst (make-vm-div :dst :r0 :lhs :r1 :rhs :r2))
         (bytes (%collect-emit-ops-bytes #'cl-cc::emit-vm-div inst)))
...
(deftest x86-emit-mod-emits-37-bytes
  ...
  (let* ((inst (make-vm-mod :dst :r0 :lhs :r1 :rhs :r2))
         (bytes (%collect-emit-ops-bytes #'cl-cc::emit-vm-mod inst)))
...
(deftest-each x86-emit-comparison-emits-bytes
  ...
  :cases (("lt"     #'cl-cc::emit-vm-lt     (make-vm-lt     :dst :r0 :lhs :r1 :rhs :r2))
          ("gt"     #'cl-cc::emit-vm-gt     (make-vm-gt     :dst :r0 :lhs :r1 :rhs :r2))
          ("le"     #'cl-cc::emit-vm-le     (make-vm-le     :dst :r0 :lhs :r1 :rhs :r2))
          ("ge"     #'cl-cc::emit-vm-ge     (make-vm-ge     :dst :r0 :lhs :r1 :rhs :r2))
          ("num-eq" #'cl-cc::emit-vm-num-eq (make-vm-num-eq :dst :r0 :lhs :r1 :rhs :r2))
          ("eq"     #'cl-cc::emit-vm-eq     (make-vm-eq     :dst :r0 :lhs :r1 :rhs :r2)))
  (emit-fn inst)
  (let ((bytes (%collect-emit-ops-bytes emit-fn inst)))
    (assert-true (> (length bytes) 0))))
...
(deftest x86-emit-logcount-emits-bytes
  ...
  (let* ((inst (make-vm-logcount :dst :r0 :src :r1))
         (bytes (%collect-emit-ops-bytes #'cl-cc::emit-vm-logcount inst)))
    (assert-true (> (length bytes) 0))))
...
(deftest x86-emit-abs-emits-bytes
  ...
  (let* ((inst (make-vm-abs :dst :r0 :src :r1))
         (bytes (%collect-emit-ops-bytes #'cl-cc::emit-vm-abs inst)))
    (assert-true (> (length bytes) 0))))
...
(deftest x86-emit-ash-emits-24-bytes
  ...
  (let* ((inst (make-vm-ash :dst :r0 :lhs :r1 :rhs :r2))
         (bytes (%collect-emit-ops-bytes #'cl-cc::emit-vm-ash inst)))
...
(deftest x86-emit-rotate-emits-bytes
  ...
  (let* ((inst (make-vm-rotate :dst :r0 :lhs :r1 :rhs :r2))
         (bytes (%collect-emit-ops-bytes #'cl-cc::emit-vm-rotate inst)))
    (assert-true (> (length bytes) 0))))
...
(deftest-each x86-emit-min-max-emits-bytes
  ...
  :cases (("min" #'cl-cc::emit-vm-min (make-vm-min :dst :r0 :lhs :r1 :rhs :r2))
          ("max" #'cl-cc::emit-vm-max (make-vm-max :dst :r0 :lhs :r1 :rhs :r2)))
  (emit-fn inst)
  (let ((bytes (%collect-emit-ops-bytes emit-fn inst)))
    (assert-true (> (length bytes) 0))))
...
(deftest x86-emit-select-emits-bytes
  ...
  (let* ((inst (make-vm-select :dst :r0 :cond-reg :r1 :then-reg :r2 :else-reg :r3))
         (bytes (%collect-emit-ops-bytes #'cl-cc::emit-vm-select inst)))
    (assert-true (> (length bytes) 0))))
