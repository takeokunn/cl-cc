...
(defun make-test-wasm-reg-map ()
  ...
  (cl-cc::make-wasm-reg-map-for-function 0))
...
(deftest wasm-tb-collect-regs-empty-instructions
  ...
  (let ((reg-map (make-test-wasm-reg-map)))
    (let ((before-count (hash-table-count (cl-cc::wasm-reg-map-table reg-map))))
      (cl-cc::collect-registers-from-instructions nil reg-map)
      (assert-= before-count (hash-table-count (cl-cc::wasm-reg-map-table reg-map))))))
...
(deftest wasm-tb-collect-regs-vm-const-touches-dst
  ...
  (let ((reg-map (make-test-wasm-reg-map))
    ...
    (cl-cc::collect-registers-from-instructions insts reg-map)
    ;; :R0 should now be in the table
    (assert-true (gethash :R0 (cl-cc::wasm-reg-map-table reg-map)))))
...
(deftest wasm-tb-collect-regs-vm-move-touches-src-and-dst
  ...
  (let ((reg-map (make-test-wasm-reg-map))
    ...
    (cl-cc::collect-registers-from-instructions insts reg-map)
    (assert-true (gethash :R1 (cl-cc::wasm-reg-map-table reg-map)))
    (assert-true (gethash :R2 (cl-cc::wasm-reg-map-table reg-map)))))
...
(deftest wasm-tb-collect-regs-assigns-unique-indices
  ...
  (let ((reg-map (make-test-wasm-reg-map))
    ...
    (cl-cc::collect-registers-from-instructions insts reg-map)
    (let ((tbl (cl-cc::wasm-reg-map-table reg-map)))
...
(deftest wasm-tb-collect-regs-idempotent-for-same-register
  ...
  (let ((reg-map (make-test-wasm-reg-map))
    ...
    (assert-= 1 (hash-table-count (cl-cc::wasm-reg-map-table reg-map)))))
...
(deftest wasm-tb-build-body-empty-blocks-emits-comment
  ...
  (let ((reg-map (make-test-wasm-reg-map))
    ...
    (let ((result (%wat-string
                   (lambda (s)
                     (cl-cc::build-trampoline-body nil label-pc-map reg-map nil s)))))
...
(deftest wasm-tb-build-body-single-block-has-block-exit
  ...
  (let* ((reg-map (make-test-wasm-reg-map))
         ...
         (bb (cl-cc::make-wasm-basic-block :label "entry" :instructions nil))
         (result (%wat-string
                  (lambda (s)
                    (cl-cc::build-trampoline-body (list bb) label-pc-map reg-map nil s)))))
...
(deftest wasm-tb-build-body-single-block-has-blk-0
  ...
  (let* ((reg-map (make-test-wasm-reg-map))
         (label-pc-map (make-hash-table :test #'equal))
         (bb (cl-cc::make-wasm-basic-block :label "start" :instructions nil))
         (result (%wat-string
                  (lambda (s)
                    (cl-cc::build-trampoline-body (list bb) label-pc-map reg-map nil s)))))
...
(deftest wasm-tb-build-body-two-blocks-has-blk-0-and-1
  ...
  (let* ((reg-map (make-test-wasm-reg-map))
         (label-pc-map (make-hash-table :test #'equal))
         (bb0 (cl-cc::make-wasm-basic-block :label "a" :instructions nil))
         (bb1 (cl-cc::make-wasm-basic-block :label "b" :instructions nil))
         (result (%wat-string
                  (lambda (s)
                    (cl-cc::build-trampoline-body (list bb0 bb1) label-pc-map reg-map nil s)))))
    (assert-true (search "$blk_0" result))
...
(deftest wasm-tb-build-all-returns-module
  ...
  (let* ((module (cl-cc::make-wasm-module-ir
                  :functions nil
                  :imports nil
                  :exports nil
                  :table-size 0)))
    (let ((result (cl-cc::build-all-wasm-functions module)))
...
(deftest wasm-tb-build-all-sets-table-size
  ...
  (let* ((f0 (cl-cc::make-wasm-function-def
              :index 0 :wat-name "$f0"
              :source-instructions nil :params nil :body nil))
         (f1 (cl-cc::make-wasm-function-def
              :index 1 :wat-name "$f1"
              :source-instructions nil :params nil :body nil))
         (module (cl-cc::make-wasm-module-ir
                  :functions (list f0 f1)
                  :imports nil
                  :exports nil
                  :table-size 0)))
    (cl-cc::build-all-wasm-functions module)
    (assert-= 2 (cl-cc::wasm-module-table-size module))))
