...
(deftest-each codegen-call-higher-order-fast-path
  ...
  :cases (("funcall" 'funcall 'cl-cc::vm-call)
          ("apply"   'apply   'cl-cc::vm-apply))
  (fn-sym inst-type)
  (let* ((ctx (make-codegen-ctx))
    ...
    (compile-ast (make-ast-call :func fn-sym
                 ...
                 ctx)
    (assert-true (codegen-find-inst ctx inst-type))
    (assert-null (codegen-find-inst ctx 'cl-cc::vm-generic-call))))
...
(deftest codegen-call-noescape-array-length-emits-const
  ...
  (let* ((ctx (make-codegen-ctx))
         ...
         (arr-reg (cl-cc::make-register ctx))
         (z-reg   (cl-cc::make-register ctx)))
    ...
    (compile-ast (make-ast-call :func 'array-length
                 ...
                 ctx)
    (let ((const-inst (codegen-find-inst ctx 'cl-cc::vm-const)))
...
(deftest optimize-ast-progn-recurses-on-forms
  ...
  (let* ((node (make-ast-progn :forms (list (make-ast-binop :op '+
         ...
         (result (cl-cc::optimize-ast node)))
    (assert-true (typep result 'cl-cc::ast-progn))
    ;; The arithmetic should be folded to an integer
    (assert-true (typep (first (cl-cc::ast-progn-forms result)) 'cl-cc::ast-int))
    (assert-= 5 (cl-cc::ast-int-value (first (cl-cc::ast-progn-forms result))))))
...
(deftest-each let-binding-special-p-dispatch
  ...
  (let ((ctx (make-codegen-ctx)))
    (when register-p
      (setf (gethash sym (cl-cc::ctx-global-variables ctx)) t))
    (assert-equal expected (cl-cc::%let-binding-special-p sym ctx))))
...
(deftest let-noescape-closure-simple-lambda
  ...
  (let* ((lam  (make-ast-lambda :params '(x)
         ...
         (result (cl-cc::%let-noescape-closure 'f lam nil nil body)))
...
(deftest let-noescape-closure-mutated-is-nil
  ...
  (let* ((lam (make-ast-lambda :params '(x)
    ...
    (assert-null (cl-cc::%let-noescape-closure 'f lam '(f) nil body))))
...
(deftest let-noescape-array-size-returns-nil-for-non-array
  ...
  (let ((expr (make-ast-int :value 5)))
    (assert-null (cl-cc::%let-noescape-array-size 'arr expr nil nil nil))))
...
(deftest let-noescape-cons-p-not-a-cons-call
  ...
  (let ((expr (make-ast-int :value 5)))
    (assert-null (cl-cc::%let-noescape-cons-p 'c expr nil nil nil))))
