...
(deftest-each phase2-write-string-arg-dispatch
  ...
  :cases (("one-arg"  (make-codegen-ctx)          (make-call 'write-string (make-quoted "hello"))                          'cl-cc::vm-princ)
          ("two-args" (make-ctx-with-vars 'stream) (make-call 'write-string (make-quoted "hello") (make-var 'stream))      'cl-cc::vm-stream-write-string-inst))
  (ctx form inst-type)
...
(deftest-each phase2-stream-reader-simple
  ...
  :cases (("read-char" 'read-char 'cl-cc::vm-read-char)
          ("read-line" 'read-line 'cl-cc::vm-read-line)
          ("read"      'read      'cl-cc::vm-read-sexp-inst))
  (fn inst-type)
  (let ((ctx (make-ctx-with-vars 'handle)))
...
(deftest-each phase2-stream-reader-eof-value
  ...
  (let ((ctx (make-ctx-with-vars 'handle 'eof-err 'eof-val)))
    ...
    (assert-true (codegen-find-inst ctx 'cl-cc::vm-eq))))
...
(deftest phase2-read-byte-eof-value
  ...
  (let ((ctx (make-ctx-with-vars 'handle 'eof-err 'eof-val)))
    ...
    (assert-true (codegen-find-inst ctx 'cl-cc::vm-read-byte))
    (assert-true (codegen-find-inst ctx 'cl-cc::vm-eq))))
...
(deftest-each phase2-newline-handler-zero-arg
  ...
  :cases (("terpri"     'terpri     'cl-cc::vm-terpri-inst)
          ("fresh-line" 'fresh-line 'cl-cc::vm-fresh-line-inst))
  (fn inst-type)
  (let ((ctx (make-codegen-ctx)))
    (compile-ast (make-call fn) ctx)
    (assert-true (codegen-find-inst ctx inst-type))))
...
(deftest-each phase2-newline-handler-stream-arg
  ...
  (let ((ctx (make-ctx-with-vars 'stream)))
    ...
    (assert-true (codegen-find-inst ctx 'cl-cc::vm-write-char))
    ;; A #\Newline constant must be loaded for the write
    (let ((nl-const (find-if (lambda (i)
                               (and (cl-cc::vm-const-p i)
                                    (eql #\Newline (cl-cc::vm-value i))))
                             (codegen-instructions ctx))))
...
(deftest-each phase2-print-single-arg
  ...
  :cases (("print"  'print  'cl-cc::vm-print-inst)
          ("prin1"  'prin1  'cl-cc::vm-prin1)
          ("princ"  'princ  'cl-cc::vm-princ))
  (fn inst-type)
  (let ((ctx (make-codegen-ctx)))
...
(deftest phase2-write-to-string-with-keywords
  ...
  (let ((ctx (make-codegen-ctx)))
    (compile-ast (make-call 'write-to-string
                            ...
                            (make-var 't))
                 ctx)
    (assert-true (codegen-find-inst ctx 'cl-cc::vm-write-to-string-inst))))
...
impl convention rather than failing on a known semantic gap."
  (let ((ctx (make-ctx-with-vars 'handle)))
    ...
    (assert-true (codegen-find-inst ctx 'cl-cc::vm-close-file))
...
(deftest phase2-unread-char-one-arg
  ...
  (let ((ctx (make-ctx-with-vars 'ch)))
    ...
    (assert-true (codegen-find-inst ctx 'cl-cc::vm-unread-char))))
...
(deftest phase2-listen-zero-arg
  ...
  (let ((ctx (make-codegen-ctx)))
    ...
    (assert-true (codegen-find-inst ctx 'cl-cc::vm-listen-inst))))
