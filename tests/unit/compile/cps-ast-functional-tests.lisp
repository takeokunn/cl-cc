...
(deftest cps-defvar-without-value-immediate-defvar
  ...
  (let* ((node (cl-cc::make-ast-defvar :name '*y* :kind 'defvar :value nil))
         (result (%cps-k node)))
...
(deftest cps-handler-case-produces-host-handler-case
  ...
  (let* ((body-node (cl-cc::make-ast-int :value 0))
         (node (cl-cc::make-ast-handler-case
...
(deftest cps-make-instance-contains-make-instance-call
  ...
  (let* ((node (cl-cc::make-ast-make-instance
                :class (cl-cc::make-ast-quote :value 'dog)
                :initargs nil))
         (result (%cps-k node)))
...
(deftest cps-set-slot-value-contains-setf-slot-value
  ...
  (let* ((node (cl-cc::make-ast-set-slot-value
                :object (cl-cc::make-ast-var :name 'obj)
                :slot 'x
...
(deftest cps-defclass-produces-progn-defclass-funcall-k
  ...
  (let* ((node (cl-cc::make-ast-defclass
...
(deftest cps-defgeneric-with-multiple-params
  ...
  (let* ((node (cl-cc::make-ast-defgeneric :name 'draw :params '(shape context)))
         (result (%cps-k node)))
...
(deftest cps-defmethod-produces-progn-defmethod-funcall-k
  ...
  (let* ((node (cl-cc::make-ast-defmethod
                :name 'area
                :params '(shape)
                :specializers '(circle)
...
(deftest cps-set-gethash-contains-setf-gethash
  ...
  (let* ((node (cl-cc::make-ast-set-gethash
                :key (cl-cc::make-ast-quote :value :x)
...
(deftest cps-mvb-with-ast-values-produces-multiple-value-bind
  ...
  (let* ((values-node (cl-cc::make-ast-values
                        :forms (list (cl-cc::make-ast-int :value 1)
                                     (cl-cc::make-ast-int :value 2))))
         (node (cl-cc::make-ast-multiple-value-bind
                :vars '(a b)
...
(deftest cps-mvb-without-ast-values-binds-primary
  ...
  (let* ((call-node (cl-cc::make-ast-call :func 'floor :args (list (cl-cc::make-ast-int :value 7))))
         (node (cl-cc::make-ast-multiple-value-bind
                :vars '(q r)
                :values-form call-node
                :body (list (cl-cc::make-ast-var :name 'q))))
         (result (%cps-k node)))
...
(deftest cps-mvc-contains-apply
  ...
  (let* ((node (cl-cc::make-ast-multiple-value-call
...
(deftest cps-mvc-collects-into-list
  ...
  (let* ((node (cl-cc::make-ast-multiple-value-call
                :func (cl-cc::make-ast-var :name 'list)
                :args (list (cl-cc::make-ast-int :value 1)
                            (cl-cc::make-ast-int :value 2))))
         (result (%cps-k node)))
...
(deftest cps-mvprog1-delivers-first-form-result
  ...
  (let* ((node (cl-cc::make-ast-multiple-value-prog1
                :first (cl-cc::make-ast-int :value 42)
                :forms (list (cl-cc::make-ast-call :func 'print
