...
(deftest binding-ignored-by-ignore-declaration
  ...
  (assert-true
   (cl-cc::%ast-let-binding-ignored-p 'x '((ignore x)))))

(deftest binding-ignored-by-ignorable-declaration
  "%ast-let-binding-ignored-p is true for (ignorable name) as well."
  (assert-true
   (cl-cc::%ast-let-binding-ignored-p 'x '((ignorable x)))))

(deftest binding-not-ignored-when-absent
  "%ast-let-binding-ignored-p is false when no matching declaration exists."
  (assert-false
   (cl-cc::%ast-let-binding-ignored-p 'x '((ignore y)))))

(deftest binding-not-ignored-in-empty-declarations
  "%ast-let-binding-ignored-p is false with no declarations."
  (assert-false
   (cl-cc::%ast-let-binding-ignored-p 'x nil)))

;;; ─── %ast-cons-call-p ─────────────────────────────────────────────────────

(deftest-each ast-cons-call-p
  "%ast-cons-call-p: true for 2-arg cons calls; false for wrong arity, non-cons, or non-call."
  :cases (("symbol-func"  (cl-cc::make-ast-call :func 'cons
                            :args (list (cl-cc::make-ast-int :value 1) (cl-cc::make-ast-int :value 2)))
                          t)
          ("var-func"     (cl-cc::make-ast-call :func (cl-cc::make-ast-var :name 'cons)
                            :args (list (cl-cc::make-ast-int :value 1) (cl-cc::make-ast-int :value 2)))
                          t)
          ("wrong-arity"  (cl-cc::make-ast-call :func 'cons
                            :args (list (cl-cc::make-ast-int :value 1)))
                          nil)
          ("non-cons"     (cl-cc::make-ast-call :func 'list
                            :args (list (cl-cc::make-ast-int :value 1) (cl-cc::make-ast-int :value 2)))
                          nil)
          ("non-call"     (cl-cc::make-ast-int :value 5)
                          nil))
  (node expected)
  (if expected
      (assert-true  (cl-cc::%ast-cons-call-p node))
      (assert-false (cl-cc::%ast-cons-call-p node))))
...
(deftest-each ast-make-array-call-p
  ...
  :cases (("symbol-func"  (cl-cc::make-ast-call :func 'make-array
                            :args (list (cl-cc::make-ast-int :value 10)))
                          t)
          ("wrong-arity"  (cl-cc::make-ast-call :func 'make-array
                            :args (list (cl-cc::make-ast-int :value 10)
                                        (cl-cc::make-ast-int :value 5)))
                          nil))
  (node expected)
  (if expected
      (assert-true  (cl-cc::%ast-make-array-call-p node))
      (assert-false (cl-cc::%ast-make-array-call-p node))))

(deftest-each ast-make-array-int-call-p
  "%ast-make-array-int-call-p: true when size is ast-int; false when size is a variable."
  :cases (("literal-size"  (cl-cc::make-ast-call :func 'make-array
                             :args (list (cl-cc::make-ast-int :value 5)))
                           t)
          ("var-size"      (cl-cc::make-ast-call :func 'make-array
                             :args (list (cl-cc::make-ast-var :name 'n)))
                           nil))
  (node expected)
  (if expected
      (assert-true  (cl-cc::%ast-make-array-int-call-p node))
      (assert-false (cl-cc::%ast-make-array-int-call-p node))))
...
(deftest-each binding-mentioned-in-body-p
  ...
  :cases (("var-in-body"    (list (cl-cc::make-ast-var :name 'x))  'x  t)
          ("empty-body"     nil                                     'x  nil)
          ("different-var"  (list (cl-cc::make-ast-var :name 'y))  'x  nil))
  (body name expected)
  (if expected
      (assert-true  (cl-cc::%binding-mentioned-in-body-p body name))
      (assert-false (cl-cc::%binding-mentioned-in-body-p body name))))
...
(deftest ast-lambda-bound-names-required-params
  ...
  (let ((node (cl-cc::make-ast-lambda
               :params '(a b c)
               :body nil)))
    (assert-equal '(a b c) (cl-cc::%ast-lambda-bound-names node))))
...
(deftest ast-lambda-bound-names-includes-rest-param
  ...
  (let ((node (cl-cc::make-ast-lambda
               ...
               :rest-param 'rest
               :body nil)))
    (let ((names (cl-cc::%ast-lambda-bound-names node)))
...
(deftest ast-lambda-bound-names-includes-optional-params
  ...
  (let ((node (cl-cc::make-ast-lambda
               ...
               :body nil)))
    (assert-true (member 'a (cl-cc::%ast-lambda-bound-names node)))))
...
(deftest ast-as-body-forms-unwraps-progn
  ...
  (let* ((f1 (cl-cc::make-ast-int :value 1))
    ...
    (assert-equal (list f1 f2) (cl-cc::%ast-as-body-forms node))))
...
(deftest ast-as-body-forms-wraps-non-progn-in-list
  ...
  (let ((node (cl-cc::make-ast-int :value 42)))
    (assert-equal (list node) (cl-cc::%ast-as-body-forms node))))
...
(deftest ast-wrap-bindings-no-bindings-single-body-returns-form
  ...
  (let ((f (cl-cc::make-ast-int :value 7)))
    (assert-eq f (cl-cc::%ast-wrap-bindings nil (list f)))))
...
(deftest ast-wrap-bindings-no-bindings-multi-body-returns-progn
  ...
  (let ((result (cl-cc::%ast-wrap-bindings
                 nil
...
(deftest ast-wrap-bindings-with-bindings-returns-ast-let
  ...
  (let ((result (cl-cc::%ast-wrap-bindings
                 (list (cons 'x (cl-cc::make-ast-int :value 1)))
                 (list (cl-cc::make-ast-var :name 'x)))))
...
integer bindings aren't candidates."
  ...
  (let* ((node (cl-cc::make-ast-let
                :bindings (list (cons 'arr (cl-cc::make-ast-call
                                            :func 'make-array
                                            :args (list (cl-cc::make-ast-int :value 3)))))
                :body (list (cl-cc::make-ast-if
                             :cond (cl-cc::make-ast-int :value 1)
                             :then (cl-cc::make-ast-call
                                    :func 'aref
                                    :args (list (cl-cc::make-ast-var :name 'arr)
                                                (cl-cc::make-ast-int :value 0)))
                             :else (cl-cc::make-ast-int :value 0)))))
         (result (cl-cc::%ast-let-sink-if-candidate node)))
    (assert-true result)))
...
(deftest sink-if-candidate-nil-for-non-if-body
  ...
  (let ((node (cl-cc::make-ast-let
    ...
    (assert-null (cl-cc::%ast-let-sink-if-candidate node))))
...
(deftest-each let-binding-special-p
  ...
  (let ((ctx (make-instance 'cl-cc::compiler-context)))
    (when register-sym
      (setf (gethash register-sym (cl-cc::ctx-global-variables ctx)) t))
    (if expected
        (assert-true  (cl-cc::%let-binding-special-p sym ctx))
        (assert-false (cl-cc::%let-binding-special-p sym ctx)))))
