...
(deftest-each codegen-slot-predicate-emits-instruction
  ...
  :cases (("boundp"     'slot-boundp     'cl-cc::vm-slot-boundp)
          ("exists-p"   'slot-exists-p   'cl-cc::vm-slot-exists-p)
          ("makunbound" 'slot-makunbound 'cl-cc::vm-slot-makunbound))
  (form inst-type)
  (let ((ctx (make-ctx-with-vars 'obj)))
    ...
    (let ((inst (codegen-find-inst ctx inst-type)))
      (assert-true inst)
      (assert-eq 'field (cl-cc::vm-slot-name-sym inst)))))
