...
(deftest ast-constant-folding-before-codegen
  ...
  (let ((arith (cl-cc::optimize-ast (cl-cc::lower-sexp-to-ast '(+ 1 2 3))))
        (strlen (cl-cc::optimize-ast (cl-cc::lower-sexp-to-ast '(string-length "hello")))))
    (assert-true (cl-cc::ast-int-p arith))
    (assert-= 6 (cl-cc::ast-int-value arith))
    (assert-true (cl-cc::ast-int-p strlen))
    (assert-= 5 (cl-cc::ast-int-value strlen))))
...
(deftest ast-partial-eval-known-defun-call
  ...
  (let ((result (cl-cc::compile-toplevel-forms
                 '((defun add1 (x) (+ x 1))
                   (add1 41)))))
    (let ((asts (cl-cc::compilation-result-ast result)))
      (assert-true (cl-cc::ast-int-p (second asts)))
      (assert-= 42 (cl-cc::ast-int-value (second asts))))))
...
(deftest codegen-let-ignore-binding-skips-own-move
  ...
  (let* ((ctx (make-codegen-ctx))
         (reg (compile-ast (make-ast-let
                           ...
                           ctx))
         (moves (remove-if-not (lambda (i) (typep i 'cl-cc::vm-move))
                               (codegen-instructions ctx))))
    (assert-true (keywordp reg))
...
(deftest codegen-let-noescape-cons-car-bypasses-vm-car
  ...
  (let* ((ctx (make-codegen-ctx))
         (reg (compile-ast (make-ast-let
                           ...
                           ctx))
         (cars (remove-if-not (lambda (i) (typep i 'cl-cc::vm-car))
                              (codegen-instructions ctx)))
         (moves (remove-if-not (lambda (i) (typep i 'cl-cc::vm-move))
                               (codegen-instructions ctx))))
    (assert-true (keywordp reg))
    (assert-null (codegen-find-inst ctx 'cl-cc::vm-cons))
...
(deftest codegen-let-noescape-array-variable-aref-bypasses-vm-aref
  ...
  (let* ((ctx (make-codegen-ctx))
    ...
    (assert-null (codegen-find-inst ctx 'cl-cc::vm-make-array))
    (assert-null (codegen-find-inst ctx 'cl-cc::vm-aref))
    (assert-true (codegen-find-inst ctx 'cl-cc::vm-num-eq))
...
(deftest codegen-let-escaped-array-aref-falls-back-to-vm-aref
  ...
  (let* ((ctx (make-codegen-ctx))
    ...
    (assert-true (codegen-find-inst ctx 'cl-cc::vm-aref))))
...
(deftest codegen-let-noescape-array-aset-bypasses-vm-make-array-and-vm-aset
  ...
  (let* ((ctx (make-codegen-ctx))
    ...
    (assert-true (keywordp reg))
    (assert-null (codegen-find-inst ctx 'cl-cc::vm-make-array))
    (assert-null (codegen-find-inst ctx 'cl-cc::vm-aset))
    (assert-null (codegen-find-inst ctx 'cl-cc::vm-aref))
    (assert-true (codegen-find-inst ctx 'cl-cc::vm-move))))
