...
(deftest ir-fmt-value-cases
  ...
  (assert-true (search "hello" (cl-cc:ir-format-value "hello")))
...
(deftest ir-print-block-variations
  ...
  (let* ((fn    (cl-cc:ir-make-function 'test))
    ...
    (let ((s (with-output-to-string (out) (cl-cc:ir-print-block entry out))))
      (assert-true (search "(none)" s)))
    ;; block with predecessor lists it
    (cl-cc:ir-add-edge entry then)
    (let ((s (with-output-to-string (out) (cl-cc:ir-print-block then out))))
      (assert-true (search "entry" s)))
    ;; block with params shows label and param
    (push v (cl-cc:irb-params join))
    (let ((s (with-output-to-string (out) (cl-cc:ir-print-block join out))))
...
(deftest ir-print-module-cases
  ...
  (let* ((mod (cl-cc:make-ir-module :functions nil))
         (s   (with-output-to-string (out) (cl-cc:ir-print-module mod out))))
