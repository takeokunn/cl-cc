...
;;;
;;; Tests for the new defopcode macro infrastructure, vm-state defstruct,
...
(deftest vm2-state-is-struct
  "make-vm-state creates a vm-state struct."
  (let ((s (cl-cc::make-vm-state)))
    (assert-true (cl-cc::vm-state-p s))))

(deftest vm2-state-register-file-is-simple-vector
  "vm-state-registers is a simple-vector of 256 elements."
  (let ((s (cl-cc::make-vm-state)))
    (assert-true (simple-vector-p (cl-cc::vm-state-registers s)))
    (assert-= cl-cc::+vm-register-count+ (length (cl-cc::vm-state-registers s)))))

(deftest vm2-state-registers-init-nil
  "All 256 registers are initialised to NIL."
  (let ((s (cl-cc::make-vm-state)))
    (dotimes (i 256)
      (assert-true (null (cl-cc::vm-reg-get s i))))))
...
(deftest vm2-state-output-stream
  ...
  (let* ((str (make-string-output-stream))
    ...
    (assert-equal str (cl-cc::vm-output-stream s))))
...
(deftest vm2-reg-set-returns-value
  ...
  (let ((s (cl-cc::make-vm-state)))
    (assert-= 99 (cl-cc::vm-reg-set s 5 99))))

(deftest vm2-reg-overwrite
  "vm-reg-set overwrites a previously written value."
  (let ((s (cl-cc::make-vm-state)))
    (cl-cc::vm-reg-set s 3 100)
    (cl-cc::vm-reg-set s 3 200)
    (assert-= 200 (cl-cc::vm-reg-get s 3))))

;;; ------------------------------------------------------------
;;; make-register returns integer
;;; ------------------------------------------------------------

(deftest make-register-returns-fixnum
  "make-register returns a fixnum, not a keyword."
  (let ((ctx (make-instance 'cl-cc::compiler-context)))
    (let ((r0 (cl-cc::make-register ctx))
      ...
      (assert-true (typep r0 'fixnum))
      (assert-true (typep r1 'fixnum))
      (assert-true (typep r2 'fixnum)))))

(deftest make-register-sequential
  "make-register allocates 0, 1, 2, ... in order."
  (let ((ctx (make-instance 'cl-cc::compiler-context)))
    (assert-= 0 (cl-cc::make-register ctx))
    (assert-= 1 (cl-cc::make-register ctx))
    (assert-= 2 (cl-cc::make-register ctx))))
...
(deftest run-vm-register-independence
  ...
  (let ((s (cl-cc::make-vm-state)))
    ...
    (let ((code (make-bytecode cl-cc::+op2-const+ 1 10  nil
                               ...
                               cl-cc::+op2-halt2+ 0 nil nil)))
      (cl-cc::run-vm code s)
      (assert-= 10 (cl-cc::vm-reg-get s 1))
      (assert-= 20 (cl-cc::vm-reg-get s 2)))))
