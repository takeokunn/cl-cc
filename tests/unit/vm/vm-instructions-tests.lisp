...
(deftest vm-instruction-readers
  ...
  (let ((inst (cl-cc::make-vm-closure :dst :r7 :label 'LBL :params '(a b)
    ...
    (assert-eq :r7 (cl-cc::vm-dst inst))
    (assert-eq 'LBL (cl-cc::vm-label-name inst))
    (assert-equal '(a b) (cl-cc::vm-closure-params inst))
    (assert-equal '(c) (cl-cc::vm-closure-optional-params inst))
    (assert-eq 'rest (cl-cc::vm-closure-rest-param inst))
    (assert-equal '((:k k)) (cl-cc::vm-closure-key-params inst))
    (assert-equal '(:r1 :r2) (cl-cc::vm-captured-vars inst))))
