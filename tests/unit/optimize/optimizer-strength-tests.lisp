...
(deftest-each opt-power-of-2-p-true-for-powers-of-two
  ...
  :cases (("two"         2)
          ...
          ("two-fifty-six" 256))
  (n)
  (assert-true (cl-cc::opt-power-of-2-p n)))
...
(deftest-each opt-power-of-2-p-false-for-non-powers
  ...
  :cases (("zero"        0)
          ...
          ("float"       4.0))
  (n)
  (assert-false (cl-cc::opt-power-of-2-p n)))
...
(deftest strength-reduce-mul-by-power-of-2-emits-ash
  ...
  (let* ((const-4 (make-vm-const :dst :r0 :value 4))
         ...
         (result  (cl-cc::opt-pass-strength-reduce (list const-4 mul))))
    ;; The mul should become ash; check no vm-mul in output
    (assert-false (some (lambda (i) (typep i 'cl-cc::vm-mul)) result))
    (assert-true  (some (lambda (i) (typep i 'cl-cc::vm-ash)) result))))
...
(deftest strength-reduce-mul-rhs-power-of-2-emits-ash
  ...
  (let* ((const-8 (make-vm-const :dst :r0 :value 8))
         ...
         (result  (cl-cc::opt-pass-strength-reduce (list const-8 mul))))
    (assert-false (some (lambda (i) (typep i 'cl-cc::vm-mul)) result))
    (assert-true  (some (lambda (i) (typep i 'cl-cc::vm-ash)) result))))
...
(deftest strength-reduce-div-by-power-of-2-emits-ash
  ...
  (let* ((const-8 (make-vm-const :dst :r0 :value 8))
         ...
         (result  (cl-cc::opt-pass-strength-reduce (list const-8 div))))
    (assert-false (some (lambda (i) (typep i 'cl-cc::vm-div)) result))
    (assert-true  (some (lambda (i) (typep i 'cl-cc::vm-ash)) result))
    ;; The shift count must be negative (right shift)
    (let ((ash-inst (find-if (lambda (i) (typep i 'cl-cc::vm-ash)) result)))
      (when ash-inst
        (let ((shift-dst (cl-cc::vm-rhs ash-inst)))
          (let ((const-inst (find-if (lambda (i)
                                       (and (typep i 'cl-cc::vm-const)
                                            (eq (cl-cc::vm-dst i) shift-dst)))
                                     result)))
            (when const-inst
              (assert-true (minusp (cl-cc::vm-value const-inst))))))))))
...
(deftest strength-reduce-mod-by-power-of-2-emits-logand
  ...
  (let* ((const-8 (make-vm-const :dst :r0 :value 8))
         ...
         (result  (cl-cc::opt-pass-strength-reduce (list const-8 mod))))
    (assert-false (some (lambda (i) (typep i 'cl-cc::vm-mod)) result))
    (assert-true  (some (lambda (i) (typep i 'cl-cc::vm-logand)) result))
...
(deftest strength-reduce-non-power-of-2-mul-passthrough
  ...
  (let* ((const-7 (make-vm-const :dst :r0 :value 7))
         (mul     (make-vm-mul   :dst :r1 :lhs :x :rhs :r0))
         (result  (cl-cc::opt-pass-strength-reduce (list const-7 mul))))
    ;; 7 has 3 bits set — logcount(7)=3 > 2, so no decomposition
    (assert-true (some (lambda (i) (typep i 'cl-cc::vm-mul)) result))))
...
(deftest strength-reduce-label-clears-constant-env
  ...
  (let* ((const-4 (make-vm-const :dst :r0 :value 4))
         ...
         (mul     (make-vm-mul   :dst :r1 :lhs :x :rhs :r0))
         (result  (cl-cc::opt-pass-strength-reduce (list const-4 lbl mul))))
...
(deftest bswap-recognition-passes-through-non-bswap
  ...
  (let* ((const (make-vm-const :dst :r0 :value 42))
         ...
         (result (cl-cc::opt-pass-bswap-recognition (list const move))))
    (assert-= 2 (length result))
    (assert-true (typep (first result)  'cl-cc::vm-const))
    (assert-true (typep (second result) 'cl-cc::vm-move))))
...
(deftest rotate-recognition-passes-through-non-rotate
  ...
  (let* ((const (make-vm-const :dst :r0 :value 10))
         ...
         (result (cl-cc::opt-pass-rotate-recognition (list const add))))
    (assert-= 2 (length result))
    (assert-true (typep (first result)  'cl-cc::vm-const))
    (assert-true (typep (second result) 'cl-cc::vm-add))))
...
(deftest rotate-recognition-collapses-rotate-idiom
  ...
  (let* ((k0  (make-vm-const :dst :rc0 :value 8))
         ...
         (result (cl-cc::opt-pass-rotate-recognition (list k0 a0 k1 a1 or0))))
    ;; Should collapse to two instructions: a vm-const + vm-rotate
    (assert-true (some (lambda (i) (typep i 'cl-cc::vm-rotate)) result))
    ;; vm-logior should not appear in the output
    (assert-false (some (lambda (i) (typep i 'cl-cc::vm-logior)) result))))
