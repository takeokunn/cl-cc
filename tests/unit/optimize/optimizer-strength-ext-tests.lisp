...
(deftest-each reassociate-commutative-p-true-for-commutative-ops
  ...
  (inst)
  (assert-true (cl-cc::opt-reassociate-commutative-p inst)))
...
(deftest-each reassociate-commutative-p-false-for-non-commutative
  ...
  (inst)
  (assert-false (cl-cc::opt-reassociate-commutative-p inst)))
...
(deftest-each copy-commutative-binop-creates-correct-type
  ...
  :cases (("vm-add"         (make-vm-add         :dst :r0 :lhs :r0 :rhs :r0) 'cl-cc::vm-add)
          ("vm-integer-add" (make-vm-integer-add  :dst :r0 :lhs :r0 :rhs :r0) 'cl-cc::vm-integer-add)
          ("vm-mul"         (make-vm-mul          :dst :r0 :lhs :r0 :rhs :r0) 'cl-cc::vm-mul)
          ("vm-logand"      (make-vm-logand       :dst :r0 :lhs :r0 :rhs :r0) 'cl-cc::vm-logand)
          ("vm-logior"      (make-vm-logior       :dst :r0 :lhs :r0 :rhs :r0) 'cl-cc::vm-logior)
          ("vm-logxor"      (make-vm-logxor       :dst :r0 :lhs :r0 :rhs :r0) 'cl-cc::vm-logxor))
  (inst expected-type)
  (let ((new-inst (cl-cc::opt-copy-commutative-binop inst :r3 :r4 :r5)))
    (assert-true (typep new-inst expected-type))
    (assert-eq :r3 (cl-cc::vm-dst new-inst))
    (assert-eq :r4 (cl-cc::vm-lhs new-inst))
    (assert-eq :r5 (cl-cc::vm-rhs new-inst))))
...
(deftest copy-commutative-binop-otherwise-returns-inst
  ...
  (let* ((inst (make-vm-sub :dst :r0 :lhs :r1 :rhs :r2))
         (result (cl-cc::opt-copy-commutative-binop inst :r9 :r8 :r7)))
...
(deftest reassociate-empty-input-returns-nil
  ...
  (assert-null (cl-cc::opt-pass-reassociate nil)))
...
(deftest reassociate-straight-line-no-consts-unchanged
  ...
  (let* ((insts (list (make-vm-add :dst :r2 :lhs :r0 :rhs :r1)
                      (make-vm-ret :reg :r2)))
         (result (cl-cc::opt-pass-reassociate insts)))
...
(deftest batch-concatenate-empty-input-returns-nil
  ...
  (assert-null (cl-cc::opt-pass-batch-concatenate nil)))
...
(deftest batch-concatenate-non-concat-passthrough
  ...
  (let* ((insts (list (make-vm-const :dst :r0 :value 1)
                      ...
                      (make-vm-ret   :reg :r1)))
         (result (cl-cc::opt-pass-batch-concatenate insts)))
    (assert-= (length insts) (length result))))
...
(deftest batch-concatenate-single-no-chain
  ...
  (let* ((inst   (make-vm-concatenate :dst :r2 :str1 :r0 :str2 :r1))
    ...
    (assert-= 1 (count-if (lambda (i) (typep i 'cl-cc::vm-concatenate)) result))))
...
(deftest batch-concatenate-packs-two-chain
  ...
  (let* ((c1 (make-vm-concatenate :dst :r2 :str1 :r0 :str2 :r1))
    ...
    (let ((packed (find-if (lambda (i) (typep i 'cl-cc::vm-concatenate)) result)))
      (when packed
        (let ((parts (cl-cc::vm-parts packed)))
...
(deftest batch-concatenate-does-not-pack-when-used-multiple-times
  ...
  (let* ((c1  (make-vm-concatenate :dst :r2 :str1 :r0 :str2 :r1))
    ...
    (assert-true (>= (count-if (lambda (i) (typep i 'cl-cc::vm-concatenate)) result) 2))))
