...
(deftest-each heap-root-inst-p-true-cases
  ...
  (inst)
  (assert-true (cl-cc::opt-heap-root-inst-p inst)))
...
(deftest-each heap-root-inst-p-false-cases
  ...
  (inst)
  (assert-false (cl-cc::opt-heap-root-inst-p inst)))
...
(deftest-each heap-root-kind-values
  ...
  (inst expected-kind)
  (assert-eq expected-kind (cl-cc::opt-heap-root-kind inst)))

(deftest heap-root-kind-nil-for-non-heap
  "opt-heap-root-kind returns NIL for non-heap instructions."
  (assert-null (cl-cc::opt-heap-root-kind (make-vm-const :dst :r0 :value 1))))
...
(deftest build-root-map-cons-sets-own-root
  ...
  (let* ((inst (make-vm-cons :dst :r0 :car-src :r1 :cdr-src :r2))
         (roots (cl-cc::opt-compute-heap-aliases (list inst))))
...
(deftest build-root-map-move-propagates-root
  ...
  (let* ((cons-inst (make-vm-cons :dst :r0 :car-src :r1 :cdr-src :r2))
         ...
         (roots (cl-cc::opt-compute-heap-aliases (list cons-inst move-inst))))
...
(deftest build-root-map-non-heap-write-kills-root
  ...
  (let* ((cons-inst (make-vm-cons :dst :r0 :car-src :r1 :cdr-src :r2))
         ...
         (roots (cl-cc::opt-compute-heap-aliases (list cons-inst add-inst))))
...
(deftest points-to-root-returns-nil-for-unknown
  ...
  (let ((pt (make-hash-table :test #'eq)))
    (assert-null (cl-cc::opt-points-to-root :r99 pt))))
...
(deftest points-to-root-returns-canonical-root
  ...
  (let ((pt (make-hash-table :test #'eq)))
    ...
    (assert-eq :r0 (cl-cc::opt-points-to-root :r0 pt))))

;;; ─── opt-interval-* arithmetic ───────────────────────────────────────────

(deftest interval-make-lo-hi
  "opt-make-interval sets lo and hi correctly."
  (let ((iv (cl-cc::opt-make-interval 3 7)))
    (assert-= 3 (cl-cc::opt-interval-lo iv))
    (assert-= 7 (cl-cc::opt-interval-hi iv))))

(deftest interval-add-produces-correct-range
  "[1,2] + [3,4] = [4,6]."
  (let* ((a (cl-cc::opt-make-interval 1 2))
         (b (cl-cc::opt-make-interval 3 4))
         (r (cl-cc::opt-interval-add a b)))
    (assert-= 4 (cl-cc::opt-interval-lo r))
    (assert-= 6 (cl-cc::opt-interval-hi r))))

(deftest interval-sub-produces-correct-range
  "[5,8] - [1,3] = [2,7]."
  (let* ((a (cl-cc::opt-make-interval 5 8))
         (b (cl-cc::opt-make-interval 1 3))
         (r (cl-cc::opt-interval-sub a b)))
    (assert-= 2 (cl-cc::opt-interval-lo r))
    (assert-= 7 (cl-cc::opt-interval-hi r))))

(deftest interval-mul-positive-ranges
  "[2,3] * [4,5] = [8,15]."
  (let* ((a (cl-cc::opt-make-interval 2 3))
         (b (cl-cc::opt-make-interval 4 5))
         (r (cl-cc::opt-interval-mul a b)))
    (assert-= 8  (cl-cc::opt-interval-lo r))
    (assert-= 15 (cl-cc::opt-interval-hi r))))
...
(deftest interval-mul-mixed-signs
  ...
  (let* ((a (cl-cc::opt-make-interval -1 2))
         ...
         (r (cl-cc::opt-interval-mul a b)))
    (assert-= -4 (cl-cc::opt-interval-lo r))
    (assert-= 8  (cl-cc::opt-interval-hi r))))
...
(deftest constant-intervals-vm-const-integer
  ...
  (let* ((insts (list (make-vm-const :dst :r0 :value 5)))
         (ivals (cl-cc::opt-compute-constant-intervals insts)))
    (let ((iv (gethash :r0 ivals)))
      (assert-true iv)
      (assert-= 5 (cl-cc::opt-interval-lo iv))
      (assert-= 5 (cl-cc::opt-interval-hi iv)))))
...
(deftest-each must-alias-cases
  ...
  (let ((alias (make-hash-table :test #'eq)))
    ...
    (if expected
        (assert-true  (cl-cc::opt-must-alias-p reg-a reg-b alias))
        (assert-false (cl-cc::opt-must-alias-p reg-a reg-b alias)))))
...
(deftest-each may-alias-cases
  ...
  (let ((alias (make-hash-table :test #'eq)))
    (loop for (k v) on entries by #'cddr do (setf (gethash k alias) v))
    (if expected
        (assert-true  (cl-cc::opt-may-alias-p reg-a reg-b alias))
        (assert-false (cl-cc::opt-may-alias-p reg-a reg-b alias)))))
...
(deftest slot-alias-key-uses-known-root
  ...
  (let ((alias (make-hash-table :test #'eq)))
    ...
    (let ((key (cl-cc::opt-slot-alias-key :r0 'x alias)))
...
(deftest slot-alias-key-falls-back-to-reg
  ...
  (let ((alias (make-hash-table :test #'eq)))
    (let ((key (cl-cc::opt-slot-alias-key :r5 'y alias)))
...
(deftest rewrite-inst-regs-replaces-source-register
  ...
  (let* ((inst (make-vm-add :dst :r2 :lhs :r0 :rhs :r1))
    ...
    (let ((new-inst (cl-cc::opt-rewrite-inst-regs inst copies)))
      ;; dst should be unchanged, lhs should be :r5
      (assert-eq :r2 (cl-cc::vm-dst new-inst))
      (assert-eq :r5 (cl-cc::vm-lhs new-inst)))))
...
(deftest rewrite-inst-regs-preserves-dst
  ...
  (let* ((inst (make-vm-add :dst :r0 :lhs :r1 :rhs :r2))
    ...
    (let ((new-inst (cl-cc::opt-rewrite-inst-regs inst copies)))
      (assert-eq :r0 (cl-cc::vm-dst new-inst)))))
...
(deftest rewrite-inst-regs-no-copies-unchanged
  ...
  (let* ((inst (make-vm-move :dst :r0 :src :r1))
         (copies (make-hash-table :test #'eq))
         (new-inst (cl-cc::opt-rewrite-inst-regs inst copies)))
    (assert-eq :r1 (cl-cc::vm-src new-inst))))
...
(deftest dead-store-elim-drops-overwritten-global-store
  ...
  (let* ((insts (list (make-vm-const      :dst :r0 :value 1)
                      ...
                      (make-vm-ret        :reg :r1)))
         (result (cl-cc::opt-pass-dead-store-elim insts)))
    ;; Only one vm-set-global should remain
    (assert-= 1 (count-if (lambda (i) (typep i 'cl-cc::vm-set-global)) result))))
...
(deftest dead-store-elim-preserves-store-before-read
  ...
  (let* ((insts (list (make-vm-const      :dst :r0 :value 1)
         ...
         (result (cl-cc::opt-pass-dead-store-elim insts)))
    (assert-= 1 (count-if (lambda (i) (typep i 'cl-cc::vm-set-global)) result))
    (assert-= 1 (count-if (lambda (i) (typep i 'cl-cc::vm-get-global)) result))))
...
(deftest store-to-load-forward-replaces-get-global-with-move
  ...
  (let* ((insts (list (make-vm-const      :dst :r0 :value 10)
                      ...
                      (make-vm-ret        :reg :r1)))
         (result (cl-cc::opt-pass-store-to-load-forward insts)))
    ;; The get-global should be replaced by a vm-move
    (assert-true (some (lambda (i) (typep i 'cl-cc::vm-move)) result))
    (assert-false (some (lambda (i) (typep i 'cl-cc::vm-get-global)) result))))
...
(deftest store-to-load-forward-preserves-when-no-pending-store
  ...
  (let* ((insts (list (make-vm-get-global :dst :r0 :name 'unknown)
                      (make-vm-ret        :reg :r0)))
         (result (cl-cc::opt-pass-store-to-load-forward insts)))
    (assert-true (some (lambda (i) (typep i 'cl-cc::vm-get-global)) result))))
