...
(deftest expander-docstring-helpers
  ...
  (let* ((body '("Doc" (foo) (bar)))
         (stripped (cl-cc::%strip-docstring body)))
    (assert-equal '((foo) (bar)) stripped)
    (assert-equal "Doc" (cl-cc::%extract-docstring body))))
...
(deftest expander-defsetf-short-form-registers-updater
  ...
  (let ((accessor 'expander-defsetf-short-accessor)
    ...
    (cl-cc::compiler-macroexpand-all `(defsetf ,accessor ,updater))
    (assert-true (gethash accessor cl-cc::*setf-compound-place-handlers*))
    (let ((result (cl-cc::compiler-macroexpand-all `(setf (,accessor obj arg) value))))
...
(deftest expander-define-symbol-macro-and-get-decoded-time
  ...
  (let* ((name 'expander-definitions-symbol-macro)
         (expansion '(+ 1 2))
         (result (cl-cc::compiler-macroexpand-all `(define-symbol-macro ,name ,expansion))))
    (assert-eq 'quote (car result))
    (assert-eq name (second result))
    (assert-equal expansion (gethash name cl-cc::*symbol-macro-table*)))
  (assert-equal '(decode-universal-time (get-universal-time))
                (cl-cc::compiler-macroexpand-all '(get-decoded-time))))
