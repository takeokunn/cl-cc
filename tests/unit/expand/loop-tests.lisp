...
(deftest loop-build-return-forms-prefers-result-form
  ...
  (assert-equal '((values foo bar))
                (cl-cc::%loop-build-return-forms '(foo bar) nil nil))
  (assert-equal '(foo)
                (cl-cc::%loop-build-return-forms '(foo) nil nil)))
...
(deftest loop-build-return-forms-falls-back-to-accumulators-and-vacuous-truth
  ...
  (assert-equal '((values a b))
                (cl-cc::%loop-build-return-forms nil '(b a) nil))
  (assert-equal '(t)
                (cl-cc::%loop-build-return-forms nil nil '((:always t))))
  (assert-null (cl-cc::%loop-build-return-forms nil nil nil)))
