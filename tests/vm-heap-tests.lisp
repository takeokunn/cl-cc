...
(deftest vm-car-extracts-value
  ...
  (let* ((state (make-instance 'vm-state))
    ...
    (assert-= (vm-reg-get state :r3) 123)))
...
(deftest vm-cdr-extracts-value
  ...
  (let* ((state (make-instance 'vm-state))
    ...
    (assert-= (vm-reg-get state :r3) 456)))
...
(deftest vm-closure-ref-idx-accesses-value
  ...
  (let* ((state (make-instance 'vm-state))
    ...
    (assert-= (vm-reg-get state :r4) 10)
    (assert-= (vm-reg-get state :r5) 20)
    (assert-= (vm-reg-get state :r6) 30)))
...
(deftest instruction->sexp-vm-make-closure
  ...
  (let ((inst (make-vm-make-closure
                             ...
                             :params '(:x :y)
                             :env-regs '(:r1 :r2))))
    (assert-equal (instruction->sexp inst) '(:make-closure :r0 :func (:x :y) :r1 :r2))))
...
(deftest instruction->sexp-vm-closure-ref-idx
  ...
  (let ((inst (make-vm-closure-ref-idx
                             ...
                             :closure :r1
                             :index 2)))
    (assert-equal (instruction->sexp inst) '(:closure-ref-idx :r0 :r1 2))))
...
(deftest sexp->instruction-vm-cons
  ...
  (let ((inst (sexp->instruction '(:cons :r0 :r1 :r2))))
    ...
    (assert-eq (vm-dst inst) :r0)
    (assert-eq (vm-car-reg inst) :r1)
    (assert-eq (vm-cdr-reg inst) :r2)))
...
(deftest sexp->instruction-vm-rplaca
  ...
  (let ((inst (sexp->instruction '(:rplaca :r0 :r1))))
    ...
    (assert-eq (vm-cons-reg inst) :r0)
    (assert-eq (vm-val-reg inst) :r1)))
...
(deftest sexp->instruction-vm-make-closure
  ...
  (let ((inst (sexp->instruction '(:make-closure :r0 :func (:x) :r1))))
    ...
    (assert-equal (vm-env-regs inst) '(:r1))))
...
(deftest sexp->instruction-vm-closure-ref-idx
  ...
  (let ((inst (sexp->instruction '(:closure-ref-idx :r0 :r1 3))))
    ...
    (assert-eq (vm-dst inst) :r0)
    (assert-eq (vm-closure-reg inst) :r1)
...
(deftest traverse-list-with-car-cdr
  ...
  (let* ((state (make-instance 'vm-state))
    ...
    (let ((list-cell (vm-reg-get state :r5)))
      ...
      (assert-= (vm-reg-get state :r11) 10)
      ...
      (assert-= (vm-reg-get state :r13) 20)
      ...
      (assert-= (vm-reg-get state :r15) 30))))
