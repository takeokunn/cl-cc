...
(defparameter *a64-emitter-table*
  (let ((ht (make-hash-table :test #'eq)))
    ...
    (setf (gethash 'vm-lt ht) #'emit-a64-vm-lt)
    (setf (gethash 'vm-gt ht) #'emit-a64-vm-gt)
    (setf (gethash 'vm-le ht) #'emit-a64-vm-le)
    (setf (gethash 'vm-ge ht) #'emit-a64-vm-ge)
    (setf (gethash 'vm-num-eq ht) #'emit-a64-vm-num-eq)
    (setf (gethash 'vm-eq ht) #'emit-a64-vm-eq)
    (setf (gethash 'vm-null-p ht) #'emit-a64-vm-null-p)
    (setf (gethash 'vm-number-p ht) #'emit-a64-vm-true-pred)
    (setf (gethash 'vm-integer-p ht) #'emit-a64-vm-true-pred)
    (setf (gethash 'vm-cons-p ht) #'emit-a64-vm-false-pred)
    (setf (gethash 'vm-symbol-p ht) #'emit-a64-vm-false-pred)
    (setf (gethash 'vm-function-p ht) #'emit-a64-vm-false-pred)
  ...
  (let ((tp (type-of inst)))
    (cond
      ((or (eq tp 'vm-label)
           ...
           (eq tp 'vm-set-global)
           (eq tp 'cl-cc/optimize::opt-vm-path-profile-record))
       nil)
      ((eq tp 'vm-const)
