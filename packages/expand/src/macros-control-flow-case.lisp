...
(our-defmacro typecase (keyform &body cases)
  ...
  (let* ((key-var (gensym "KEY"))
         ...
         (default-form (if default-clause
                           ...
                           nil))
         ;; Dispatch strategy: decision tree for nested/subtype-related clauses,
         ;; otherwise a linear typep chain. Both rely on TYPEP, which understands
         ;; the full CL type lattice (e.g. FIXNUM <: INTEGER). The former CASE-based
         ;; jump table over VM-TYPE-OF was removed: its tag vocabulary (:INTEGER)
         ;; never matched VM-TYPE-OF's actual symbol output ('FIXNUM), so it always
         ;; fell through to the default arm — silently wrong for every typed clause.
         (dispatch-form (cond
