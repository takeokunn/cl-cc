...
(defun lazy-map (f ls) (lazy-seq (let ((p (force ls))) (when p (cons (funcall f (car p)) (lazy-map f (cdr p)))))))
(defun lazy-filter (pred ls) (lazy-seq (loop for p = (force ls) while p when (funcall pred (car p)) return (cons (car p) (lazy-filter pred (cdr p))) do (setf ls (cdr ls)))))
