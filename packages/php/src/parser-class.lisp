       ...
       (let ((rest (%php-skip-type-annotation stream)))
          (if (eq (php-peek-type rest) :T-VAR)
              (multiple-value-bind (var-tok rest2) (php-consume rest)
                (when (and rest2 (eq (php-peek-type rest2) :T-OP)
                           (equal "=" (php-peek-value rest2)))
                  (setf rest2 (%php-skip-expression-like (cdr rest2) '(:T-SEMI) nil)))
                (values (make-ast-slot-def :name (php-var-sym (php-tok-value var-tok)))
                        (php-skip-semis rest2)))
              (values nil rest))))
      ;; const NAME = value; — class constants are skipped as non-slot members.
      ((and (eq (php-peek-type stream) :T-KEYWORD)
            (eq (php-peek-value stream) :const))
       (values nil (php-skip-semis (%php-skip-expression-like (cdr stream) '(:T-SEMI) nil))))
