...
(deftest "%lsa-try-coalesce fails when source is still active (end > current start)"
  ;; Source ends at 10; new interval starts at 5 → source is still live.
  (let* ((state (make-test-lsa-state))
         (src-int (make-lsa-test-interval :src 0 10))
         (new-int (make-lsa-test-interval :new 5 15 :coalesce-with :src)))
    (setf (interval-phys-reg src-int) :r0)
    (setf (gethash :src (lsa-interval-map state)) src-int)
    (let ((result (%lsa-try-coalesce state new-int)))
      (is (null result))
      (is (null (interval-phys-reg new-int))))))

(deftest "%lsa-try-coalesce fails when coalesce-with is nil"
  (let* ((state (make-test-lsa-state))
         (int (make-lsa-test-interval :v 0 10)))
    (is (null (%lsa-try-coalesce state int)))))

(deftest "%lsa-try-coalesce fails when FP class differs"
  ;; Source is GPR, new is FP → classes differ → no coalescing.
  (let* ((state (make-test-lsa-state))
         (src-int (make-lsa-test-interval :src 0 5 :fp-p nil))
         (new-int (make-lsa-test-interval :new 5 10 :coalesce-with :src :fp-p t)))
    (setf (interval-phys-reg src-int) :r0)
    (setf (gethash :src (lsa-interval-map state)) src-int)
    (setf (lsa-active state) (list src-int))
...
(deftest "%lsa-allocate-from-pool assigns first available register"
...
(deftest "%lsa-allocate-from-pool removes assigned register from pool"
  (let* ((state (make-test-lsa-state :free-regs '(:r0 :r1 :r2)))
         (interval (make-lsa-test-interval :v 0 10))
         (cc (make-target-desc
               :name        :x86-64
               :gpr-names   #(:rdi :rsi :rdx)
               :arg-regs    '(:rdi :rsi)
               :ret-reg     :rax
               :fp-arg-regs '(:xmm0)
               :fp-ret-reg  :xmm0
...
(deftest "%lsa-spill-current sets spill-slot on interval"
  (let* ((state    (make-test-lsa-state))
         (interval (make-lsa-test-interval :v 0 10)))
...
(deftest "%lsa-spill-current increments spill-count"
  (let* ((state    (make-test-lsa-state))
         (interval (make-lsa-test-interval :v 0 10)))
    (let ((before (lsa-spill-count state)))
      (%lsa-spill-current state interval)
...
(deftest "%lsa-interval-pool returns free-regs for gpr interval"
  (let* ((state    (make-test-lsa-state :free-regs '(:r0 :r1) :free-fp-regs '(:xmm0)))
         (interval (make-lsa-test-interval :v 0 10 :fp-p nil)))
    (is (equal '(:r0 :r1) (%lsa-interval-pool state interval)))))

(deftest "%lsa-interval-pool returns free-fp-regs for fp interval"
  (let* ((state    (make-test-lsa-state :free-regs '(:r0) :free-fp-regs '(:xmm0 :xmm1)))
         (interval (make-lsa-test-interval :v 0 10 :fp-p t)))
    (is (equal '(:xmm0 :xmm1) (%lsa-interval-pool state interval)))))

(deftest "%lsa-set-interval-pool updates free-regs for gpr interval"
  (let* ((state    (make-test-lsa-state :free-regs '(:r0) :free-fp-regs '(:xmm0)))
         (interval (make-lsa-test-interval :v 0 10 :fp-p nil)))
    (%lsa-set-interval-pool state interval '(:r0 :r1 :r2))
    (is (equal '(:r0 :r1 :r2) (lsa-free-regs state)))
    (is (equal '(:xmm0) (lsa-free-fp-regs state)))))

(deftest "%lsa-set-interval-pool updates free-fp-regs for fp interval"
  (let* ((state    (make-test-lsa-state :free-regs '(:r0) :free-fp-regs '(:xmm0)))
         (interval (make-lsa-test-interval :v 0 10 :fp-p t)))
    (%lsa-set-interval-pool state interval '(:xmm0 :xmm1))
    (is (equal '(:xmm0 :xmm1) (lsa-free-fp-regs state)))
    (is (equal '(:r0) (lsa-free-regs state)))))
...
(deftest "%return-value-preferred-reg returns ABI ret-reg for return-value interval"
  (let* ((cc       (make-minimal-cc :ret-reg :rax))
         (interval (make-lsa-test-interval :v 0 10))
         (free-regs '(:rax :rbx :rcx)))
    (setf (interval-return-value-p interval) t)
    (is (eq :rax (%return-value-preferred-reg interval cc free-regs)))))

(deftest "%return-value-preferred-reg returns nil when interval is not return-value"
  (let* ((cc       (make-minimal-cc :ret-reg :rax))
         (interval (make-lsa-test-interval :v 0 10))
         (free-regs '(:rax :rbx)))
    (is (null (%return-value-preferred-reg interval cc free-regs)))))

(deftest "%return-value-preferred-reg returns nil when ret-reg not in free pool"
  (let* ((cc       (make-minimal-cc :ret-reg :rax))
         (interval (make-lsa-test-interval :v 0 10))
         (free-regs '(:rbx :rcx)))
    (setf (interval-return-value-p interval) t)
...
(deftest "%return-value-preferred-reg uses fp-ret-reg for fp return-value interval"
  (let* ((cc       (make-minimal-cc :fp-ret-reg :xmm0))
         (interval (make-lsa-test-interval :v 0 10 :fp-p t))
         (free-regs '(:xmm0 :xmm1)))
    (setf (interval-return-value-p interval) t)
    (is (eq :xmm0 (%return-value-preferred-reg interval cc free-regs)))))
