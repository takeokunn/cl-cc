...
(deftest-each x86-emit-byte-cases
  ...
  (input expected)
  (let ((bytes (%x86-collect-bytes (lambda (s) (cl-cc/emit::emit-byte input s)))))
...
(deftest x86-emit-multi-byte-le
  ...
  (let ((dw (%x86-collect-bytes (lambda (s) (cl-cc/emit::emit-dword #xDEADBEEF s))))
        (qw (%x86-collect-bytes (lambda (s) (cl-cc/emit::emit-qword #x0102030405060708 s)))))
...
(deftest x86-jmp-rel32-le-offset
  ...
  (let ((bytes (%x86-collect-bytes (lambda (s) (cl-cc/emit::emit-jmp-rel32 256 s)))))
...
(deftest x86-vm-program-leaf-red-zone-spills-skip-rbp-frame
  ...
  (let* ((prog (cl-cc/vm::make-vm-program
                ...
                :result-register :R0
                :leaf-p t))
         (ra (cl-cc/emit::make-regalloc-result :assignment (make-hash-table :test #'eq)
                                          :spill-count 1
                                          :instructions (cl-cc/vm::vm-program-instructions prog)))
         (bytes (let ((cl-cc/emit::*current-regalloc* ra))
