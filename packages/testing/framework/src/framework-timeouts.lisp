...
(defun %normalize-positive-timeout (timeout)
  "Return TIMEOUT when it is a positive integer, otherwise NIL."
...
By default, existing explicit per-test timeouts are preserved."
  (let ((normalized (%normalize-positive-timeout timeout)))
    (unless normalized
      (error "Timeout must be a positive integer: ~S" timeout))
    (%map-test-registry
     (lambda (name plist)
...
When RECURSIVE is true, descendant suite tests are updated as well.
By default, existing explicit per-test timeouts are preserved."
  (let ((normalized (%normalize-positive-timeout timeout)))
    (unless normalized
      (error "Timeout must be a positive integer: ~S" timeout))
