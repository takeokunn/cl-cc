...
(deftest lower-make-instance-form
  ...
  (let ((node (lower '(make-instance 'point :x 1 :y 2))))
    (assert-true (cl-cc/ast::ast-make-instance-p node))
    (assert-= 2 (length (cl-cc/ast::ast-make-instance-initargs node)))))
...
(deftest lower-slot-value-form
  ...
  (let ((node (lower '(slot-value obj 'name))))
    (assert-true (cl-cc/ast::ast-slot-value-p node))
    (assert-eq 'name (cl-cc/ast::ast-slot-value-slot node))))
