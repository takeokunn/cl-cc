...
(deftest opt-pass-data-integrity
  ...
  (assert-equal '(:prolog-rewrite :call-site-splitting :devirtualize :closure-capture-dedup :closure-thunk-sharing)
                (subseq cl-cc/optimize::*opt-default-convergence-pass-keys* 0 5))
  (assert-eq :inline (sixth cl-cc/optimize::*opt-default-convergence-pass-keys*))
  (assert-eq :overflow-check-elim (seventh cl-cc/optimize::*opt-default-convergence-pass-keys*))
  (assert-eq :sccp (eighth cl-cc/optimize::*opt-default-convergence-pass-keys*))
  (assert-eq :cons-slot-forward (ninth cl-cc/optimize::*opt-default-convergence-pass-keys*))
