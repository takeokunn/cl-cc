...
Returns (func-infos forward-env) where func-infos is (name label closure-reg box-reg params)."
  (let ((func-infos nil)
    ...
    (let ((forward-env (mapcar (lambda (info) (cons (first info) (fourth info))) func-infos)))
      (values func-infos forward-env))))
...
(defun %labels-fill-closure (info bindings ctx forward-env old-env)
  ...
  (destructuring-bind (name func-label closure-reg box-reg params) info
    (let* ((binding-data  (find name bindings :key #'first))
           ...
           (param-regs    (loop for _ in params collect (make-register ctx)))
  ...
  (let* ((tail             (ctx-tail-position ctx))
    ...
    (unwind-protect
         (multiple-value-bind (func-infos forward-env)
           ...
           (let ((func-env (mapcar (lambda (b) (cons (first b) (lookup-var ctx (first b)))) bindings)))
...
Returns the regalloc-rewritten program."
  (let* ((instructions (vm-program-instructions program))
         (ra (allocate-registers instructions cl-cc/target:*x86-64-target*)))
