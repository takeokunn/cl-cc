...
(deftest egraph-add-deduplicates
  ...
  (let* ((eg  (make-test-egraph))
         ...
         (id1 (cl-cc/optimize::egraph-add eg 'const)))
    (assert-= id0 id1)))
...
(deftest egraph-stats-empty
  ...
  (let* ((eg    (make-test-egraph))
         (stats (cl-cc/optimize::egraph-stats eg)))
...
(deftest egraph-stats-after-add
  ...
  (let* ((eg (make-test-egraph)))
    ...
    (let ((stats (cl-cc/optimize::egraph-stats eg)))
...
(defun egraph-find-dst-inst (insts dst)
  (find dst insts
        :key (lambda (inst) (ignore-errors (cl-cc/vm::vm-dst inst)))
        :test #'eq))
...
(deftest enode-memo-key-nullary
  ...
  (let ((node (cl-cc/optimize::make-e-node :op 'const :children nil)))
    (assert-equal '(const) (cl-cc/optimize::enode-memo-key node))))

(deftest enode-memo-key-binary
  "enode-memo-key includes child IDs in the key."
  (let ((node (cl-cc/optimize::make-e-node :op 'add :children '(0 1))))
    (assert-equal '(add 0 1) (cl-cc/optimize::enode-memo-key node))))
...
(deftest egraph-class-has-op-p-true
  ...
  (let* ((eg (make-test-egraph))
         (id (cl-cc/optimize::egraph-add eg 'const)))
    (assert-true (cl-cc/optimize::egraph-class-has-op-p eg id 'const))))

(deftest egraph-class-has-op-p-false
  "egraph-class-has-op-p is false when no node in the e-class has the given op."
  (let* ((eg (make-test-egraph))
         (id (cl-cc/optimize::egraph-add eg 'const)))
    (assert-false (cl-cc/optimize::egraph-class-has-op-p eg id 'add))))
